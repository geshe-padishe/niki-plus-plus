...
{
    // std::cout << "sort inside" << std::endl;
    // typename FordContainer<FordPair<int>, ContainerType>::iterator itb;
    // for (itb = arr.begin(); itb != arr.end(); ++itb) {
    //    std::cout << (*itb).toString() << " ";
    // }
    //std::cout << std::endl;
    for (size_t i = 0; i < arr.size() - 1; i++)
    {
        if (arr[i + 1] < arr[i])
        {
            //std::cout << "swaping: " << arr[i + 1].toString() << " and " << arr[i].toString() << std::endl;
            std::swap(arr[i], arr[i + 1]);
            i = 0;
        }
    }
    //JE SAIS PAS POURQUOI LA BOUCLE DAVANT NE TRIE PAS LE PREMIER DONC JE LA RELANCE ET PAF CA TRIE :$ :@ :!
    for (size_t i = 0; i < arr.size() - 1; i++)
    {
        if (arr[i + 1] < arr[i])
        {
    ...
    //    std::cout << (*itb).toString() << " ";
    // }
    // std::cout << "end sort inside" << std::endl;
}
...
void fordJohnsonSort(ContainerType<Type>& arr, bool should_print)
{
    ...
    // std::cout << "printing rest ";
    // for (size_t i = 0; i < reste_v.size(); i++)
    //     std::cout << reste_v[i].toString();
    // std::cout << " end print rest" << std::endl;
